...
import com.team3.otboo.domain.user.dto.Request.UserSearchParams;
...
public class UserServiceImpl implements UserService {
	...
	public UserResponse createUser(UserCreateRequest request) {
		...
		userFollowerCountRepository.save(UserFollowerCount.init(user.getId(), 0L));
		userFollowingCountRepository.save(UserFollowingCount.init(user.getId(), 0L));
	...
	public UserDtoCursorResponse getUsers(UserSearchParams userSearchParams) {
		...
		String nextCursor = getNextCursor(users, userSearchParams.limit(),
			userSearchParams.sortBy());

		Long totalCount = userRepository.totalCount(userSearchParams.emailLike(),
			userSearchParams.roleEqual(), userSearchParams.locked());
		...
		return UserDtoCursorResponse.of(userDtos, nextCursor, nextIdAfter, hasNext, totalCount,
			userSearchParams.sortBy(), userSearchParams.sortDirection());
	}
